...
export default function OrgCard({ org }: OrgProps) {
    ...
    return (
        <div className='flex flex-col gap-y-2 border-2 p-4 border-customBlack rounded-lg w-[300px] h-[350px]'>
            ...
            {/* <Image src={org.avatar as string} alt='Avatar' width={250} height={250}/> */}
            <div className='flex flex-col gap-y-3 h-full'>
                <h3 onClick={() => router.push(`/orgs/${org.id}`)} className='text-lg font-bold cursor-pointer'>{org.name.slice(0,30)}{org.name.length > 30 ? "...": ""}</h3>
                <p className='text-sm'>{org.description.slice(0,200)}{org.description.length > 200 ? "...": ""}</p>
            </div>
        </div>
    )
